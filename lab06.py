...
## Program: 5
## Name: Marco Mera
## Email:mmv7v@umkc.edu
##
## PROBLEM : There was a couple problems like finding the right values, returns and outputs, and finally small misplacements of spaces or any other type of characters.
##
## ALGORITHM : 
##      1. def get_school(library_card)
##      2.def get_grade (library_card)
##      3.def character_value (c)
##      4.def get_check_digit (library_card)
##      5.def verify_check_digit (library_card)
##      6.def main()
##      7.if __name__=="__main__"
##
## ERROR HANDLING: There was just an unexpected OEL error due to string errors in the LabTest. Otherwise, there were only syntax, index, value and type errors. 
##      
##
## OTHER COMMENTS:
##      This lab was quite challenging.
##
########################################################################
...
def verify_check_digit (library_card):
    ...
    for i in range(5):
        if library_card[i] < 'A' or library_card[i] > 'Z':
            msg = "The first 5 characters must be A-Z, the invalid character is at " \
            + str(i) +" is " + library_card[i]
            return (False, msg)

    for i in range(7,10):
        if library_card[i] < '0' or library_card[i] > '9':
            msg = "The last 3 characters must be 0-9, the invalid character is at "\
                    + str(i) +" is " + library_card[i]
            return (False, msg)
